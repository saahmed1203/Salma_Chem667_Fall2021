...
def frame_processing(): # function to process a single frame
    ...
    #cv2.rectangle(colorIM, (10, 2), (100,20), (255,255,255), -1)
    #cv2.putText(colorIM, str(cap.get(cv2.CAP_PROP_POS_FRAMES)), (15, 15),
    ...
    fgMask = backSub.apply(colorIM) #background subtracted image
    #grayIM = cv2.cvtColor(fgMask, cv2.COLOR_BGR2GRAY)  # convert color to grayscale image       
    #blurIM=cv2.medianBlur(grayIM,BLUR)                  # blur image to fill in holes to make solid object
    ret,binaryIM = cv2.threshold(fgMask,thresh,255,cv2.THRESH_BINARY) # threshold image to make pixels 0 or 255
    ...
    cv2.imshow('FG Mask', cv2.resize(fgMask,VGA))       # display backsub image
    cv2.waitKey(1)
...
def doButton(): #determines functions of each button
    global thresh, MIN_AREA, MAX_AREA, skip_im, save, run, play
